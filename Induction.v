...
(** Now prove commutativity of multiplication.  (You will probably
    ...
    handy. *)
Theorem mult_fact_1 : forall m n : nat, m + m * n = m * S n.
Proof.
  intros n m.
    induction n as [|n'].
    Case "n = 0".
...
Proof.
  intros n m. 
    ...
    Case "n = S n'".
      simpl. rewrite -> IHn'. rewrite <- mult_fact_1. reflexivity. 
Qed.
(** [] *)
...
Proof.
  ...
  induction n as [|n'].
    Case "n = 0".
      simpl. reflexivity.
    Case "n = S n'".
      assert (H: negb (evenb (S (S n'))) = negb (evenb n')).
        simpl. reflexivity.
      rewrite -> H.
      rewrite -> IHn'.
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
  intros b c. destruct b, c as [b'|c'].
  simpl. reflexivity.
  simpl. reflexivity.
  simpl. reflexivity.
  simpl. reflexivity.
...
Proof.
  ...
  Case "p = S p'".
    ...
    rewrite -> IHp'. rewrite <- mult_fact_1. 
    rewrite <- plus_assoc.
    rewrite -> plus_swap. 
    rewrite -> plus_comm. 
    rewrite -> plus_assoc.
    rewrite <- plus_assoc.
    rewrite <- mult_fact_1.
    assert (H: (m * p' + m) = (m + m * p')).
      SCase "(m * p' + m) = (m + m * p')p = S p'".
      rewrite <- plus_comm. reflexivity.
